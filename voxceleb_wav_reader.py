...
def read_aolme_structure(directory, data_type):
    print('to do')
    aolme_test = pd.DataFrame()
    if data_type == 'wavs':
        aolme_test['filename'] = find_wavs(directory)
    elif data_type == 'feats':
        aolme_test['filename'] = find_feats1(directory)
    else:
        raise NotImplementedError
    aolme_test['filename'] = aolme_test['filename'].apply(lambda x: x.replace('\\', '/')) # normalize windows paths
    num_speakers = len(os.listdir(directory))
    logging.info('Found {} files with {} different speakers.'.format(str(len(aolme_test)).zfill(7), str(num_speakers).zfill(5)))
    logging.info(aolme_test.head(10))
    return aolme_test